				...
				if (msgList.size() == 0){
					...
					do{
						...
						try{				
							...
							if (msgIdx < -1)
								throw new Exception("Input must be greater than -1");
						}catch(Exception e){
							System.out.println(
								"Invalid Input. It must be positive numeral less than " + msgList.size() + "or -1");						
						}
					}while(msgIdx < -1);
					...
					if(messageBox.equalsIgnoreCase("inbox")){
						...
						msgData.setMessageIdToRetreive(messageId);
