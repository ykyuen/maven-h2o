...
public class DateUtil
{
	...
	{
		return DateUtil.date2UTC(dateTime.getTime(), dateTime.getTimeZone());
	}
	...
	getCurrentUTCDateTime() 
	{
		// Suppress exception.
		try { 
			return DateUtil.date2UTC(new Date(), TimeZone.getDefault()); 
	...
	{	
		return new Timestamp(DateUtil.GMT2Date(dateTime).getTime());
	}
	...
	{	
		return DateUtil.GMT2Date(dateTime).getTime();
	}
	...
	GMT2Calender(String dateTime) throws UtilitiesException
	{
		...
		cal.setTimeInMillis(DateUtil.GMT2Date(dateTime).getTime());
		return cal;
	}
